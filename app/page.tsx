      ...
      <div className="min-h-screen" style={{ 
        ...
        <section className="max-w-7xl mx-auto px-6 pt-40 pb-20">
          <div className="text-center">
            ...
            >
              <div className="relative bg-gray-800 rounded-3xl p-4">
                <div className="flex items-center gap-4">
                  <div className="flex-1 relative">
                    <textarea
                      ...
                      className="w-full h-16 p-4 bg-transparent text-white placeholder-gray-400 focus:outline-none resize-none text-lg text-left"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className="w-10 h-10 bg-gray-300 hover:bg-gray-200 disabled:bg-gray-700 disabled:cursor-not-allowed rounded-full flex items-center justify-center transition-all duration-200"
                  >
                    <svg className="w-5 h-5 text-gray-800 disabled:text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <motion.div
              ...
              transition={{ delay: 0.4 }}
                className="flex flex-wrap justify-center gap-3 mb-32"
            >
                {examplePrompts.map((example, index) => (
                  <button
                    key={index}
                    onClick={() => setPrompt(example)}
                    className="bg-transparent border border-white/30 rounded-lg px-4 py-2 hover:bg-white/10 hover:border-white/50 transition-all duration-200 cursor-pointer text-white/70 hover:text-white text-sm backdrop-blur-sm"
                  >
                    {example}
                  </button>
                ))}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
      <motion.div
        ...
        className="bg-gray-800 rounded-2xl p-8 max-w-md mx-4"
        style={{ 
          fontFamily: 'Inter, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif'
        }}
      >
        <h2 className="text-2xl font-bold text-white mb-4">Ready to create your voice agent?</h2>
        <p className="text-gray-300 mb-6">Sign up or sign in to start building your custom voice agent.</p>
          
        <div className="flex flex-col space-y-3">
          <button
            onClick={handleSignUp}
            className="w-full bg-black text-white px-6 py-3 rounded-lg hover:bg-gray-900 transition-colors font-medium"
          >
            Create Account
          </button>
          <button
            onClick={handleSignIn}
            className="w-full bg-white text-gray-900 px-6 py-3 rounded-lg hover:bg-gray-100 transition-colors font-medium"
          >
            Sign In
          </button>
          <button
